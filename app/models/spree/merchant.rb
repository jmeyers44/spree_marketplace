class Spree::Merchant < Spree::Base
  extend FriendlyId
  ...
  after_create :send_welcome, if: -> { SpreeMarketplace::Config[:send_merchant_email] }
  ...
  def send_welcome
    Spree::MerchantMailer.welcome(id).deliver_later!
  ...
  def set_commission
    unless changes.key?(:commission_flat_rate)
      self.commission_flat_rate = SpreeMarketplace::Config[:default_commission_flat_rate]
    end
    unless changes.key?(:commission_percentage)
      self.commission_percentage = SpreeMarketplace::Config[:default_commission_percentage]
    end
  end
end